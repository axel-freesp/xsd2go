...
<xsl:template match="xsd:schema">
	<xsl:value-of select="concat('/*', xsd:annotation[1], '*/', $NL)"/>
	...
	<xsl:apply-templates select="xsd:complexType | xsd:annotation[position() &gt; 1]"/>
...
<xsl:template match="xsd:complexType">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:value-of select="'/*'"/>
			<xsl:apply-templates select="." mode="PrettyCopy"/>
			<xsl:value-of select="concat($NL, '*/ ', $NL)"/>
		</xsl:otherwise>
	</xsl:choose>
	<!--xsl:message>
...
<xsl:template match="xsd:complexType" mode="output">
	...
	<xsl:choose>
		<xsl:when test="$mode = 'definition'">
			<xsl:value-of select="concat('type ', $go-name, ' struct {', $NL)"/>
...
<xsl:template mode="PrettyCopy" match="*">
    <xsl:param name="indent" select="$NL"/>
	<xsl:variable name="name" select="local-name()"/>
    <xsl:value-of select="$indent"/>
    <xsl:element name="{$name}">
        <xsl:copy-of select="attribute::*"/>
        <xsl:if test="child::*">
            <xsl:apply-templates mode="PrettyCopy" select="child::*">
